...
// Test the showNotification function
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function handleCellClick(event) {
        ...
        if (gameBoard[index] === "" && !checkWinner()) {
            ...
            // Check for a tie after every move
            if (checkIfTie()) {
                console.log("All cells are occupied. It's a tie!");
    ...
    function checkIfTie() {
        // If every cell is occupied and there is no winner, it's a tie
...
function checkIfTie() {
    ...
    for (const cell of gameBoard) {
        if (cell === "") {
            emptyCellCount++;
            if (emptyCellCount > 1) {
                return false; // If more than one empty cell is found, it's not a tie
            }
        }
    }
    if (emptyCellCount === 1) {
        // Create a container for the tie message and overlay
        const container = document.createElement("div");
        container.classList.add("tie-container");
        const overlay = document.getElementById("overlay");

        // Create the tie message element
        const tieMessage = document.createElement("div");
        tieMessage.textContent = "It's a tie!";
        tieMessage.classList.add("tie-message"); // Optionally, add a class for additional styling

        // Append the tie message to the container
        container.appendChild(tieMessage);
        overlay.style.display = "block";

        // Append the container to the body or another suitable container
        document.body.appendChild(container);

        // Remove the tie message and container after a timeout
        setTimeout(() => {
    ...
    function setGameBoardToTie() {
        gameBoard = Array.from({ length:  30 }, (_, index) => index %  2 ===  0 ? "X" : "O");
    }
